...
const BRAND_NAME = process.env.BRAND_NAME || "Sales Shakthi";
const WHATSAPP_NUMBER = process.env.WHATSAPP_NUMBER || "918801028315";
const PAYMENT_AMOUNT_INR = Number(process.env.PAYMENT_AMOUNT_INR || 1999);
...
const PROGRAMS = {
  "sales-shakthi": {
    ...
    description: "11 Days Magical Sales Registration",
  },
};
...
function validateLead(body) {
  const clean = {
    programId: String(body.programId || "sales-shakthi").trim(),
  ...
  if (clean.profession.length < 2) {
    errors.profession = "Profession is required.";
  }

  if (clean.details.length > 1000) {
    errors.details = "Details must be below 1000 characters.";
  }
...
app.get("/api/health", (req, res) => {
  res.json({
    success: true,
    message: "Sales Shakthi Razorpay backend running",
...
app.post("/api/payment/create-order", async (req, res) => {
  try {
    ...
    const leadId = "DSS-" + uuidv4().slice(0, 8).toUpperCase();
...
app.post("/api/payment/verify", async (req, res) => {
  try {
    ...
    const index = leads.findIndex(
      (item) =>
        item.id === leadId && item.razorpayOrderId === razorpay_order_id
    );
...
app.listen(PORT, () => {
  console.log(`Sales Shakthi backend running on port ${PORT}`);
});
